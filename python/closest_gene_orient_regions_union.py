...
def two_way(file):
  gene_orientation = pychipseq.human.peaks.ClosestGeneOrientatedPeaks("Region")
  ...
  for line in f:
    ...
    if id != pychipseq.text.NA:
      if g1 != pychipseq.text.NA and g2 == pychipseq.text.NA:
        groups1.add(id)
      elif g1 == pychipseq.text.NA and g2 != pychipseq.text.NA:
        groups2.add(id)
      elif g1 != pychipseq.text.NA and g2 != pychipseq.text.NA:
        groupsi.add(id)
      else:
...
def three_way(file):
  gene_orientation = pychipseq.human.peaks.GeneOrientatedPeaks("Region")
  gene_orientation.load_annotations(file)
  ...
  for line in f:
    ...
    if id != pychipseq.text.NA:
      if p1 != pychipseq.text.NA and p2 == pychipseq.text.NA and p3 == pychipseq.text.NA:
        g1.add(id)
      ...
      if p1 != pychipseq.text.NA and p2 != pychipseq.text.NA:
        g12.add(id)

      if p1 != pychipseq.text.NA and p3 != pychipseq.text.NA:
        g13.add(id)
      
      if p2 != pychipseq.text.NA and p3 != pychipseq.text.NA:
        g23.add(id)
  ...
  rk1 = pychipseq.sample.get_sample_id(group1)
  rk2 = pychipseq.sample.get_sample_id(group2)
  rk3 = pychipseq.sample.get_sample_id(group3)
...
def gene_orient_peaks(file):
  ...
  sample_column = pychipseq.text.find_index(header, pychipseq.headings.SAMPLE, 7)
