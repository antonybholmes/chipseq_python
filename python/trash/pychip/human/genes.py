...
class GeneOrientatedPeaks(object):
  ...
  def __init__(self, type):
    ...
    self.expression_list.append(expression.RnaSeqGeneSUD10WTvsD83vExpression())
    self.expression_list.append(expression.RnaSeqGeneSUD10WTvsStopExpression())
    self.expression_list.append(expression.RnaSeqGeneSUD10D83vsStopExpression())
  ...
  def print_header(self):
    ...
    sys.stdout.write("\t" + self.type + " " + headings.CENTROMERE)
  ...
  def mir_orient_peak(self, mir):
    ...
    sys.stdout.write("\t" + genomic.get_closest_tss(self.collapsed_tss[mir]))
...
class RefSeqGenes(genes.RefSeqGenes):
    def __init__(self):
        super().__init__(human.annotation.REFSEQ_FILE)
...
class AnnotatePeak(genes.AnnotatePeak):
    ...
    def __init__(self, 
                 ...
                 bin_size=10000):
        super().__init__(type,
             human.tss.RefSeqAnnotation(prom_ext_5p, prom_ext_3p, bin_size),
             RefSeqGenes(),
             human.tss.RefSeqTss(prom_ext_5p, prom_ext_3p),
             human.tss.RefSeqEnd(prom_ext_5p, prom_ext_3p),
             prom_ext_5p,
             prom_ext_3p,
             bin_size)
        
        # annotations specific to human
        self.annotation_modules.append(human.genomic.SimpleTandemRepeats())
        self.annotation_modules.append(human.genomic.EncodeBlacklist())
        self.annotation_modules.append(human.genomic.GiuliaBlacklist())
