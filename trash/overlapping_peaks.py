...
for file in files:
  ...
  if matcher != None:  
    ...
    if matcher != None:  
      ...
      sys.stderr.write("Wrong type of file: " + file + "\n")
  ...
  if "Peaks" in file:
    ...
    p_col = lib.text.find_index(tokens, "P-value")
    score_col = lib.text.find_index(tokens, "Score")
  ...
  for line in f:
    ...
    location = id + "=" + location.chr + ":" + str(location.start) + "-" + str(location.end)
...
location_core_map = lib.peaks.overlapping_peaks(files, ids)
