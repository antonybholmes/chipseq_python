...
def two_way(file):
  gene_orientation = lib.human.regions.ClosestGeneOrientatedRegionIntersections()
  ...
  closest_id_column = lib.text.find_index(header, lib.headings.CLOSEST_REFSEQ_ID)
  ...
  for line in f:
    ...
    if tokens[id_column] != lib.text.NA:
      id = tokens[id_column]
    elif tokens[closest_id_column] != lib.text.NA:
...
def three_way(file):
  ...
  for line in f:
    ...
    if id != lib.text.NA:
      if g1 != lib.text.NA and g2 != lib.text.NA and g3 != lib.text.NA:
        groupsi.add(id)
